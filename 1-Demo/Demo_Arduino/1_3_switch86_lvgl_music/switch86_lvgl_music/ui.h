

#ifndef _JINGCAI_3_2_DARK_UI_H
#define _JINGCAI_3_2_DARK_UI_H

#ifdef __cplusplus
extern "C" {
#endif

    #include "lvgl.h"

#include "ui_helpers.h"
#include "ui_events.h"
void rote_bl_Animation( lv_obj_t *TargetObject, int delay);
// SCREEN: ui_Screen1
void ui_Screen1_screen_init(void);
extern lv_obj_t *ui_Screen1;
extern lv_obj_t *ui_Panel2;
extern lv_obj_t *ui_Label2;
void ui_event_Button2( lv_event_t * e);
extern lv_obj_t *ui_Button2;
extern lv_obj_t *ui_Label1;
extern lv_obj_t *ui_Keyboard2;
extern lv_obj_t *ui_Panel1;
extern lv_obj_t *ui_Dropdown2;
extern lv_obj_t *ui_Button1;
extern lv_obj_t *ui_Label4;
extern lv_obj_t *ui_TextArea2;
extern lv_obj_t *ui_Keyboard1;
extern lv_obj_t *ui_Panel18;
extern lv_obj_t *ui_Label38;
extern lv_obj_t *ui_Spinner1;
extern lv_obj_t *ui_Image9;
extern lv_obj_t *ui_Label39;
// SCREEN: ui_Screen2
void ui_Screen2_screen_init(void);
extern lv_obj_t *ui_Screen2;
extern lv_obj_t *ui_Panel3;
extern lv_obj_t *ui_Label3;
void ui_event_Button3( lv_event_t * e);
extern lv_obj_t *ui_Button3;
extern lv_obj_t *ui_Label5;
extern lv_obj_t *ui_Keyboard3;
extern lv_obj_t *ui_Label7;
extern lv_obj_t *ui_Panel8;
void ui_event_Button4( lv_event_t * e);
extern lv_obj_t *ui_Button4;
extern lv_obj_t *ui_Image1;
extern lv_obj_t *ui_Label6;
void ui_event_Button5( lv_event_t * e);
extern lv_obj_t *ui_Button5;
extern lv_obj_t *ui_Image2;
extern lv_obj_t *ui_Label8;
void ui_event_Button9( lv_event_t * e);
extern lv_obj_t *ui_Button9;
extern lv_obj_t *ui_Image3;
extern lv_obj_t *ui_Label17;
// SCREEN: ui_Screen3
void ui_Screen3_screen_init(void);
extern lv_obj_t *ui_Screen3;
extern lv_obj_t *ui_Panel4;
extern lv_obj_t *ui_Label9;
void ui_event_Button6( lv_event_t * e);
extern lv_obj_t *ui_Button6;
extern lv_obj_t *ui_Label10;
extern lv_obj_t *ui_Keyboard4;
extern lv_obj_t *ui_Colorwheel2;
extern lv_obj_t *ui_ImgButton2;
// SCREEN: ui_Screen4
void ui_Screen4_screen_init(void);
extern lv_obj_t *ui_Screen4;
extern lv_obj_t *ui_Panel5;
extern lv_obj_t *ui_Label11;
void ui_event_Button7( lv_event_t * e);
extern lv_obj_t *ui_Button7;
extern lv_obj_t *ui_Label12;
void ui_event_Button15( lv_event_t * e);
extern lv_obj_t *ui_Button15;
extern lv_obj_t *ui_Label19;
extern lv_obj_t *ui_Image10;
extern lv_obj_t *ui_Keyboard5;
extern lv_obj_t *ui_Label14;
extern lv_obj_t *ui_Label20;
extern lv_obj_t *ui_Image5;
extern lv_obj_t *ui_Label18;
extern lv_obj_t *ui_Slider2;
extern lv_obj_t *ui_Button11;
extern lv_obj_t *ui_Image6;
extern lv_obj_t *ui_Button12;
extern lv_obj_t *ui_Button10;
extern lv_obj_t *ui_Label21;
extern lv_obj_t *ui_Image7;
extern lv_obj_t *ui_Button13;
extern lv_obj_t *ui_Button14;
extern lv_obj_t *ui_Roller2;
extern lv_obj_t *ui_Panel9;
extern lv_obj_t *ui_Panel11;
extern lv_obj_t *ui_Label23;
extern lv_obj_t *ui_Panel13;
extern lv_obj_t *ui_Panel10;
extern lv_obj_t *ui_Label24;
extern lv_obj_t *ui_Label25;
extern lv_obj_t *ui_Label22;
extern lv_obj_t *ui_Image8;
extern lv_obj_t *ui_Panel7;
extern lv_obj_t *ui_Label26;
extern lv_obj_t *ui_Label29;
extern lv_obj_t *ui_Label30;
extern lv_obj_t *ui_Image4;
extern lv_obj_t *ui_Button17;
extern lv_obj_t *ui_Label28;
extern lv_obj_t *ui_Panel12;
extern lv_obj_t *ui_Label27;
extern lv_obj_t *ui_Spinner2;
extern lv_obj_t *ui_Panel15;
extern lv_obj_t *ui_Slider3;
extern lv_obj_t *ui_Label32;
extern lv_obj_t *ui_Button18;
extern lv_obj_t *ui_Label33;
extern lv_obj_t *ui_Panel16;
extern lv_obj_t *ui_Label34;
extern lv_obj_t *ui_Button19;
extern lv_obj_t *ui_Label35;
extern lv_obj_t *ui_Roller1;
// SCREEN: ui_Screen5
void ui_Screen5_screen_init(void);
extern lv_obj_t *ui_Screen5;
extern lv_obj_t *ui_Panel14;
extern lv_obj_t *ui_Label31;
void ui_event_Button16( lv_event_t * e);
extern lv_obj_t *ui_Button16;
extern lv_obj_t *ui_Label36;
extern lv_obj_t *ui_Panel17;
extern lv_obj_t *ui_Label15;
extern lv_obj_t *ui_Label16;
extern lv_obj_t *ui_Switch2;
extern lv_obj_t *ui_Label37;
extern lv_obj_t *ui_Panel6;
extern lv_obj_t *ui_Label13;
extern lv_obj_t *ui_Slider1;
extern lv_obj_t *ui____initial_actions0;

LV_IMG_DECLARE( ui_img_logo_png);   // assets\logo.png
LV_IMG_DECLARE( ui_img_music_icon_png);   // assets\music_icon.png
LV_IMG_DECLARE( ui_img_light_icon_png);   // assets\light_icon.png
LV_IMG_DECLARE( ui_img_setting_icon_png);   // assets\setting_icon.png
LV_IMG_DECLARE( ui_img_light_icon_dark_png);   // assets\light_icon_dark.png
LV_IMG_DECLARE( ui_img_sdcard_icon_png);   // assets\sdcard_icon.png
LV_IMG_DECLARE( ui_img_music_b_icon_png);   // assets\music_b_icon.png
LV_IMG_DECLARE( ui_img_play_icon_png);   // assets\play_icon.png
LV_IMG_DECLARE( ui_img_pre_icon_png);   // assets\pre_icon.png
LV_IMG_DECLARE( ui_img_next_icon_png);   // assets\next_icon.png
LV_IMG_DECLARE( ui_img_pause_icon_png);   // assets\pause_icon.png
LV_IMG_DECLARE( ui_img_normal_play_icon_png);   // assets\normal_play_icon.png
LV_IMG_DECLARE( ui_img_vol_icon_png);   // assets\vol_icon.png
LV_IMG_DECLARE( ui_img_list_play_icon_png);   // assets\list_play_icon.png
LV_IMG_DECLARE( ui_img_list_pause_icon_png);   // assets\list_pause_icon.png
LV_IMG_DECLARE( ui_img_success_png);   // assets\success.png

LV_FONT_DECLARE( ui_font_Font1YHall16);
LV_FONT_DECLARE( ui_font_Font1YHall20);
LV_FONT_DECLARE( ui_font_Font1YHall24);
LV_FONT_DECLARE( ui_font_Font1YHall28);

void ui_init(void);

#ifdef __cplusplus
} /*extern "C"*/
#endif

#endif
